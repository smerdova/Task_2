...
class TestCreateOrder:
    ...
    def test_create_order_with_auth_with_ingredients_positive_result(self, user):
        payload = {
            "ingredients": ["61c0c5a71d1f82001bdaaa70", "61c0c5a71d1f82001bdaaa6d"] 
            }
        response_order = requests.post(paths.ORDER_URL, data=payload, headers={'authorization': user['accessToken']})
    ...
    def test_create_order_with_auth_without_ingredients_negative_result(self, user):              
        payload = {
            "ingredients": [] 
            }
        response_order = requests.post(paths.ORDER_URL, data=payload, headers={'authorization': user['accessToken']})
    ...
    def test_create_order_with_auth_with_incorrect_hash_negative_result(self, user):           
        payload = {
            "ingredients": ["12345"] 
            }
        response_order = requests.post(paths.ORDER_URL, data=payload, headers={'authorization': user['accessToken']})
